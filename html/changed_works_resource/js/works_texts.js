...
hrc_slider_detail_1_value = 
  '일반적인 환경에서 촬영된 신체 동작을 <br>' +
  '2D와 3D로 정확하고 빠르게 <br>' +
  '추출합니다. 이를 통해 머리부터 <br>' +
  '발끝까지 다양한 키포인트의 <br>' +
  '위치와 자세 정보를 얻을 수 있습니다. <br>' +
  '원하는 데이터 포맷에 따라 <br>' +
  '카메라 좌표계 또는 그라운드 기준<br>'+
  '좌표계 데이터로 제공됩니다.';
hrc_slider_detail_2_value = 
  '영상 속 인물이 흐리거나 사물에 <br>'+
  '가려져도 괜찮습니다. <br>' + 
  '필요한 이미지를 추정하여 생성하는 <br>'+
  '기술로 한정적인 상황에서도 <br> ' +
  '가장 자연스러운 신체의 움직임을 <br>'+
  '만들어냅니다.'
hrc_slider_detail_3_value = 
  '플레이스에이의 Human Recognition <br>'+
  'Soultion AI Engine은 3천만 장 이상의 <br>' +
  '데이터로 학습되어 별도의 부착물이나<br> ' +
  '특수한 환경 구축 없이 고품질 <br>'+
  '모션 데이터를 얻을 수 있습니다. <br>' +
  '4-8대의 일반적인 RGB 카메라만으로 <br>'+
  '기존 스튜디오 촬영에 버금가는 <br>' +
  '정밀한 3D 모션 캡처 서비스를 제공합니다.'
hac_slider_detail_1_value = 
  '추출한 신체 모션 데이터를 3D 캐릭터의 <br>'+
  '움직임으로 쉽게 변환할 수 있습니다. <br>' +
  '이미지를 바탕으로 아바타를 생성하거나 <br>'+
  '원하는 캐릭터를 불러와 모션 데이터와 <br>'+
  '결합합니다. 플레이스에이의 avatar retargeting<br>' +
  '기술로 서로 다른 비율과 체형의 <br>' +
  '캐릭터들을 동일한 동작으로 자연스럽고 <br>'+
  '매끄럽게 표현해보세요.'
hac_slider_detail_2_value = 
  '영상 속 사람의 미세한 표정까지 <br>'+
  '인식하여 3D 캐릭터의 표정으로 <br>' +
  '전환합니다. AI 기술을 통해 실시간으로<br>'+
  '표현하는 캐릭터의 표정은 <br>' +
  '가상 환경에서도 실감 나는 <br>'+
  '커뮤니케이션을 가능하게 합니다.';
orc_slider_detail_1_value = 
  '영상 속 다양한 물체들의 움직임을 <br>'+
  '실시간으로 분석합니다. <br>' +
  '프레임 단위로 동시에 물체들을 <br>'+
  '추적하여 각각 어느 방향으로  <br>' +
  '이동하고 있는지 확인할 수 있습니다.' ;
orc_slider_detail_2_value = 
  '다양한 의상 및 패션 아이템 이미지에<br>' +
  '특화된 검색 기술로, 유사한 이미지 <br>' +
  '또는 아이템을 탐색합니다. 특징이 <br>'+
  '되는 물체의 속성을 추출하고 <br>' +
  '데이터베이스에서 최적의 아이템을 <br>' +
  '추천하거나 생성하는 알고리즘이 <br>' +
  '적용되어 필요한 가상 아이템을 <br>' +
  '손쉽게 만들 수 있도록 도와줍니다.';
...
hrc_bottom_sub_1_value =
  ...
  '얻을 수 있습니다. 높은 비용의 장치가 별도로 필요하지 않기 때문에 모션 인식' +
  '기술을 활용한 다양한 서비스로 응용할 수 있습니다.'

hrc_bottom_sub_2_value = 
  '70FPS의 빠른 속도로 3D 위치 및 자세를 인식하여 실시간 서비스를' +
  '가능하게 합니다. 배경과 지면을 함께 인식하고 환경을 분석하는' +
  '플레이스에이의 독보적인 기술력은 별도의 후처리 없이 실시간으로 정확하고' +
  '자연스러운 움직임을 표현해냅니다.';

hrc_bottom_sub_3_value = 
  '원하는 동작이 담긴 영상만 있다면 모션 캡처를 위해' +
  '스튜디오를 대여하거나 전문 배우를 섭외할 필요가 없습니다.' +
  '수중 촬영처럼 제약이 많은 환경이나 현실에서는 재현 불가능한' +
  '특정 캐릭터의 움직임까지 영상만으로 편리하게' +
  '모션 데이터를 추출할 수 있습니다.'

hac_bottom_sub_4_value = 
  'UNITY, UNREAL과 같은 게임 엔진은 물론, MAYA, MAX와 같은' +
  '렌더링 엔진에서도 사용할 수 있도록 FBX 출력 파일을 제공합니다.' +
  '기존에 사용하던 익숙한 툴에서 데이터를 가져와 원하는 형태로 바로 적용해보세요.'

hac_bottom_sub_5_value = 
  '얼굴 표정의 변화와 신체의 움직임이 설정한 3D 캐릭터에 실시간으로 반영되어' +
  '내가 원하는 모습 그대로 실감나게 표현됩니다. 변환되는 캐릭터의 체형에 따라' +
  '움직임뿐만 아니라 걸음걸이, 속도까지 자동으로 보정하여 후보정 없이도' +
  '즉각적인 인터랙션이 가능합니다. '

orc_bottom_sub_6_value = 
  '신체의 움직임 이외에도 주변 사물과 환경을 이루는 물체의 움직임을' +
  '추적할 수 있습니다. 움직임이 이루어지는 공간에 대한 정보를' +
  '넓은 시야에서 처리함으로써 더욱 역동적인 가상 환경을 구축할 수 있습니다.'

orc_bottom_sub_7_value = 
  '플레이스에이의 다양한 실시간 인식 기술은 움직임, 환경, 아이템 등을' +
  '종합적으로 복제하여 궁극적인 가상 세계를 완성하게 합니다.' +
  '한 번의 촬영만으로 원하는 많은 것들을 디지털 공간으로 편리하게 옮길 수 있습니다.';
